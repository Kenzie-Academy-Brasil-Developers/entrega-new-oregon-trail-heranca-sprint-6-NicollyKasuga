...
Given(
  "o passageiro de nome <name> que está com saúde <isHealthy> e tem <food> refeições",
  function (dataTable) {
...
Then("ele não deverá ser adicionada à lista de passageiros", function () {
  let filter = carroca.passengers.filter(
    passenger => passenger.name === triedToComeAboard.name
  );
...
Then(
  "devo responder que a quantidade de assentos disponíveis é {int}",
  function (int) {
